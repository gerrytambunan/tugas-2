<x-app-layout>
    <x-slot name="title">Semua Admin</x-slot>
                        ...
                        <td>
                            @if (!empty($user->getRoleNames()))
                                @foreach ($user->getRoleNames() as $v)
                                    <label
                                        class="badge badge-{{ $v == 'Admin' ? 'danger' : 'primary' }}">{{ $v }}</label>
                                @endforeach
                            @endif
                        </td>
                        <td class="text-center">
                            <!-- <button type="button" class="btn btn-info mr-1 info" data-name="{{ $user->name }}"
                                data-username="{{ $user->username }}" data-roles="{{ $user->getRoleNames() }}"
                                data-created="{{ $user->created_at->format('d-M-Y H:m:s') }}">
                                <i class="fas fa-eye"></i>
                            </button> -->
                            <a href="{{ route('admin.member.edit', $user->id) }}" class="btn btn-primary btn-sm edit">Edit</a>
                            <form action="{{ route('admin.member.delete', $user->id) }}" style="display: inline-block;"
                                method="POST">
                                @csrf
                                <button type="button" class="btn btn-danger btn-sm ml-1 delete">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @empty
